...
import { Shield, Target, BookOpen, Award, TrendingUp, CheckCircle, Clock, Lock } from "lucide-react";
...
const userProgress = {
  riskScore: 45, rank: 3, totalEmployees: 10, trainingsCompleted: 6,
  totalTrainings: 10, quizAvgScore: 78, streak: 4, simulationsDetected: 2, simulationsTotal: 3,
};
      ...
      <div className="space-y-6 p-6">
            ...
            <CardContent className="p-6">
              <div className="flex flex-col items-center gap-4 sm:flex-row">
                <Avatar className="h-16 w-16">
                  <AvatarFallback className="bg-gradient-to-br from-rht-violet to-rht-violet-light text-lg text-white">
                    {currentUser.name.split(" ").map((n) => n[0]).join("")}
                  </AvatarFallback>
                </Avatar>
                <div className="text-center sm:text-left">
                  <h2 className="text-xl font-bold">{currentUser.name}</h2>
                  <p className="text-sm text-muted-foreground">{currentUser.email} — {currentUser.org}</p>
                </div>
                <div className="flex gap-2 sm:ml-auto">
                  {badges.filter((b) => b.earned).map((badge) => (
                    <motion.div key={badge.id} whileHover={{ scale: 1.15, rotate: 10 }}
                      className="flex h-10 w-10 items-center justify-center rounded-full bg-rht-orange/10 text-lg" title={badge.name}
                    >
                      {badge.icon}
                    </motion.div>
                  ))}
        ...
        <StaggerContainer className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { icon: TrendingUp, label: "Classement", value: `#${userProgress.rank}`, suffix: `/${userProgress.totalEmployees}`, color: "rht-violet", textColor: "" },
            { icon: Target, label: "Phishing détectés", value: `${userProgress.simulationsDetected}/${userProgress.simulationsTotal}`, color: "cyber-green", textColor: "text-cyber-green" },
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <FadeIn>
            <Card>
              <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold">Progression des formations</CardTitle></CardHeader>
              <CardContent>
                <div className="mb-4 space-y-1">
                  ...
                  <Progress value={(userProgress.trainingsCompleted / userProgress.totalTrainings) * 100} className="h-3" />
                </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className={`text-sm ${completed ? "text-muted-foreground" : "font-medium"}`}>{module.title}</p>
                        ...
                        {completed ? (
                          ...
                          <Badge className="border-0 bg-rht-violet/10 text-rht-violet-light text-[10px]"><Clock className="mr-1 h-3 w-3" />En cours</Badge>
                        ) : (
          ...
          <div className="space-y-6">
            <FadeIn delay={0.1}>
              <Card>
                <CardHeader className="pb-3"><CardTitle className="text-sm font-semibold">Mes badges</CardTitle></CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 gap-3">
                    {badges.map((badge) => (
                      <motion.div key={badge.id} whileHover={badge.earned ? { y: -3, scale: 1.02 } : {}}
                        className={`rounded-xl border p-4 text-center transition-all ${badge.earned ? "border-rht-orange/20 bg-rht-orange/5" : "opacity-40"}`}
                      >
                        <div className="mb-2 text-3xl">{badge.icon}</div>
                        <p className="text-sm font-semibold">{badge.name}</p>
                        <p className="text-[10px] text-muted-foreground">{badge.description}</p>
                        {badge.earned && <Badge className="mt-2 border-0 bg-cyber-green/10 text-cyber-green text-[10px]">Obtenu</Badge>}
                      </motion.div>
                    ))}
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-2">
                    <Award className="h-4 w-4 text-rht-orange" />
